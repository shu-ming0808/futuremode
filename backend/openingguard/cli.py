...
def _compact(result: dict[str, Any] | None) -> str:
    if not result:
        return "尚未執行"
    lines = [
        f"run_id: {result['run_id']}",
        f"scenario: {result['scenario']} ({result['label']})",
        f"synthetic: {result['synthetic_assumption']}",
    ...
    if result["recommended"]:
        ...
        lines.append(f"\nrecommendation: 08:50 預熱至 {rec['workers']} workers")
    else:
...
def interactive(runs: int) -> None:
    ...
    while True:
        ...
        print(f"{BOLD}[a]{RESET} 用 Agent 跑 high pressure  "
