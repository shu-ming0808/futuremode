...
輸出額外包含 `scale_down_first_step_seconds`、`scale_down_base_seconds`、`rebound_scale_up_count`、`budget_exhausted_warning` 與每秒 `controller_state`，供 Notebook 說明何時及為何縮容。
...
- 數值政策來自版本化的 `event_budget_v2.json`。
...
| `event` | 當時已公開事件與時間戳 | Agent 分類後套用固定政策排程；之後使用共同縮容控制器 | 專案方法 |
...
5. 報 SLO 成功率、壞日機率 CI、Queue、worker-minutes、正常／尖峰時段 idle、ready lead time、縮容時間、反彈次數與下游／預算警告。
6. 另外掃描尖峰 RPS、持續時間與訊號 lead time，揭露方法在哪些區域有效或失敗。
7. 不因結果不好而換 held-out seeds 或刪除 case。

Notebook 預設 `RUN_EXPERIMENTS=False`、`RUN_SENSITIVITY=False`、`RUN_LLM=False`，避免誤觸大量運算或付費 API。每個 code cell 前都有 Markdown 說明。
